...
export default function AdminParceiros() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let error;
    if (editingItem) {
      ({ error } = await supabase.from('partners').update(payload).eq('id', editingItem.id));
    } else {
  ...
  return (
    <AdminLayout title="Parceiros">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <p className="text-muted-foreground">Gerencie os parceiros</p>
        <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) resetForm(); }}>
          <DialogTrigger asChild>
            <Button><Plus className="h-4 w-4 mr-2" />Novo Parceiro</Button>
          </DialogTrigger>
          <DialogContent className="max-w-lg">
            <DialogHeader><DialogTitle>{editingItem ? 'Editar' : 'Novo'} Parceiro</DialogTitle></DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <Label>Nome *</Label>
                <Input value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required placeholder="Nome do parceiro" />
              </div>
